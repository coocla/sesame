...
class AMQPReceiver(object):
    ...
    def connect(self):
        ...
        """
        self.EXCHANGE = self.agentData.rabbitmq_down_exchange
        self.QUEUE = self.agentData.rabbitmq_down_queue
        self.ROUTING_KEY = ".%s." % self.agentData.agent_id
        ...
        parameters = pika.ConnectionParameters(host=self.agentData.rabbitmq_host,
                ...
                ssl=self.agentData.rabbitmq_ssl,
                channel_max=1000, connection_attempts=3,
                retry_delay=5, socket_timeout=10, heartbeat_interval=10 )
        logger.debug('Connecting to %s:%s' % (self.agentData.rabbitmq_host, self.agentData.rabbitmq_port))
    ...
    def on_connection_closed(self, connection, reply_code, reply_text):
        ...
        if self._closing:
            ...
            self.connectionStatus = ConnectionStatus.DISCONNECTED
            self.disconnectedTime = datetime.now()
    ...
    def on_message(self, unused_channel, basic_deliver, properties, body):
        ...
        try:
            ...
            msgPath = "%s/%s-%s-%s-s%s-%s" % (self.receivedQueuePath, timestamp,
                    event.EventUUID, event.EventUser, event.EventID, event.StepID)
            FileUtil.writeContent(msgPath, body)
    ...
    def run(self):
        ...
        """
        os.system("mkdir -p %s" % self.receivedQueuePath)
        self._connection = self.connect()
        self._connection.ioloop.start()
